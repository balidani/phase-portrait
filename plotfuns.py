...
def phase_portrait(fun, delta=0.01, box=(-1, 1, -1, 1),
    cmap=cm.hsv, plot_modulus=False):
    ...
    return gen_portrait(Z, fZ, box, cmap=cmap, plot_modulus=plot_modulus)
...
def gen_portrait(Z, fZ, box, cmap=cm.hsv, plot_modulus=False):
    ...
    if plot_modulus:
        res = mapper.to_rgba(modulus)
    else:
        res = mapper.to_rgba(phase)

    # plt.imshow(imvals, origin='lower', extent=tuple(box))
    return res
